...
from dataloader import SingledirectionalOneShotIterator, TestDataset, TrainDataset
from models.baselines import Baseline
from models.models import Models
from models.models_extra import ExtraModels
from models.models_nln import NLNModel
from models.models_variants import Variants
from util import eval_tuple, flatten_query, list2tuple, parse_time, set_global_seed
...
def evaluate(
    model, tp_answers, fn_answers, args, dataloader, query_name_dict, mode, step, writer
):
    """
...
def main(args):
    ...
    if args.evaluate_union == "DM":
        assert (
            args.geo == "beta"
        ), "only BetaE supports modeling union using De Morgan's Laws"
    ...
    args.save_path = os.path.join(
        prefix, args.data_path.split("/")[-1], args.tasks, args.geo
    )
    if args.geo in ["box"]:
        ...
        tmp_str = "g-{}".format(args.gamma)
    elif args.geo == "mlp2vector":
        tmp_str = "g-{}".format(args.gamma)
    elif args.geo == "mlpAttention":
        tmp_str = "g-{}".format(args.gamma)
    elif args.geo == "mlpHyperE":
        tmp_str = "g-{}".format(args.gamma)
    elif args.geo == "cnn":
        tmp_str = "g-{}".format(args.gamma)
    elif args.geo == "nln":
    ...
    if args.do_train:
        ...
        train_iterators = []
        for query_structure in train_queries:
            train_queries_i = defaultdict(set)
            train_queries_i[query_structure] = train_queries[query_structure]
            train_queries_i = flatten_query(train_queries_i)
            train_iterator_i = SingledirectionalOneShotIterator(
                DataLoader(
                    TrainDataset(
                        train_queries_i,
                        nentity,
            ...
            train_iterators.append(train_iterator_i)
    ...
    elif args.geo in ["mlp2vector", "mlpAttention", "mlpHyperE"]:
        model = Variants(
            nentity=nentity,
            nrelation=nrelation,
            hidden_dim=args.hidden_dim,
