...
class ExtraModels(nn.Module):
    def __init__(self, nentity, nrelation, hidden_dim, gamma,
                 geo, test_batch_size=1,
                 use_cuda=False,
                 query_name_dict=None,
                 mlp_mode=None):
        super(ExtraModels, self).__init__()
        ...
        if self.geo == 'cnn':
            self.notNN = CNN(self.entity_dim)
            self.projectionNN = CNN2(self.entity_dim)
            self.andNN = CNN2(self.entity_dim)
            self.orNN = CNN2(self.entity_dim)
